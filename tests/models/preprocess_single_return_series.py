...
def preprocess(x: pd.DataFrame) -> Tuple[pd.DataFrame, int]:
    y = x.copy()
...
def model(z = None, num_obs_total = None) -> None:
    ...
    mu = sample('mu', dists.Normal().expand_by((1,)).to_event(1))
    sigma = sample('sigma', dists.InverseGamma(1.).expand_by((1,)).to_event(1))
