...
write.csv(original.roc$auc, "./r_outputs/original_auc.csv")
...
write.csv(orig.res.sens.opt[1], "./r_outputs/original_sens_auc.csv")
write.csv(orig.res.sens.opt[2], "./r_outputs/original_sens_sens.csv")
write.csv(orig.res.sens.opt[3], "./r_outputs/original_sens_spec.csv")
...
write.csv(orig.res.bal.opt[1], "./r_outputs/original_bal_auc.csv")
write.csv(orig.res.bal.opt[2], "./r_outputs/original_bal_sens.csv")
write.csv(orig.res.bal.opt[3], "./r_outputs/original_bal_spec.csv")

###################################################
## load synthetic data
epsilons <- c(1.0, 2.0, 4.0)
nruns <- 10
...
for (epsilon in epsilons) {
  ...
  for (seed in c(0:(nruns-1))) {
    ...
    synthetic.data.fname <- sprintf("./full_model/syn_data_seed%d_eps%.1f.csv", seed, epsilon)
    synthetic.data <- as.data.frame(read.csv(synthetic.data.fname, na.strings = ""), stringAsFactors=F)
    synthetic.data$X <- NULL
    synthetic.data[synthetic.data=='Não Realizado'] <- NA
    synthetic.data[synthetic.data=='not_done'] <- NA
    synthetic.data[synthetic.data=='<1000'] <- 500
    
    # convert string values to factors
    synthetic.data <- synthetic.data[,names(original.data)]
    synthetic.data[original.ind] <- lapply(synthetic.data[original.ind], factor)
...
# save results
write.csv(basic.auc.df, file="./r_outputs/basic_auc_full_model.csv")

write.csv(sens.auc.df, file="./r_outputs/sens_auc_full_model.csv")
write.csv(sens.Sens.df, file="./r_outputs/sens_sens_full_model.csv")
write.csv(sens.Spec.df, file="./r_outputs/sens_spec_full_model.csv")

write.csv(bal.auc.df, file="./r_outputs/bal_auc_full_model.csv")
write.csv(bal.Sens.df, file="./r_outputs/bal_sens_full_model.csv")
write.csv(bal.Spec.df, file="./r_outputs/bal_spec_full_model.csv")
