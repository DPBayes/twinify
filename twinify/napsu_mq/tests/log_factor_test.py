...
from twinify.napsu_mq.log_factor import LogFactorTorch as LogFactor
...
class LogFactorTest(unittest.TestCase):
    ...
    def test_log_factor_marginalise(self):
        ...
        for i in range(2):
            for j in range(2):
                self.assertEqual(m0.values[i, j], torch.log(
                    torch.exp(af.values[0, i, j]) + torch.exp(af.values[1, i, j]) + torch.exp(af.values[2, i, j])))

        m1 = af.marginalise(1)
        self.assertSetEqual(set(m1.scope), {0, 4})
        for i in range(3):
            for j in range(2):
                self.assertEqual(m1.values[i, j],
                                 torch.log(torch.exp(af.values[i, 0, j]) + torch.exp(af.values[i, 1, j])))
