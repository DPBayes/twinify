...
class NAModel(dist.Distribution):
	...
	#def __init__(self, base_dist, na_logit=0.0, validate_args=None):
		self._base_dist = base_dist
		self._na_prob = na_prob
		#self._na_logit = na_logit
		super(NAModel, self).__init__()

	def log_prob(self, value):
		log_probs = dist.Bernoulli(probs = self._na_prob).log_prob(np.isnan(value))
		#log_probs = dist.Bernoulli(logits = self._na_logit).log_prob(np.isnan(value))
